...
class Curl {
    ...
    private function streamCallback($ch, $data) {
        if (!method_exists($this->response, "streamCallback")) {
            throw new \Exception("method streamCallback not exist");
        }
        $this->response->streamCallback($ch, $data);
        return strlen($data);
    }
    /**
     * 设置流回执
     ...
     * @date 2026-04-23
     */
    public function setStream_old($callback = null) {
        $this->stream = true;
        $use_callback = null;
        $this->setOpt(CURLOPT_RETURNTRANSFER, false);
        if (empty($callback) && $this->response instanceof ResponseAbstract) {
            $use_callback = $this->response;
            $this->stream_response = $this->response;
        } elseif (is_array($callback)) {
            $classObj = $callback[0];
            $function = $callback[1];
            $this->stream_response = $classObj;
            $use_callback = [$classObj, $function];
        }
        if (empty($use_callback)) {
    ...
    public function setStream() {
        $this->stream = true;
        $this->setOpt(CURLOPT_RETURNTRANSFER, false);
        $this->setOpt(CURLOPT_WRITEFUNCTION, [$this, "streamCallback"]);
    }
    ...
    public function get($url, $conf = []) {
        return $this->request($url, "GET", $conf);
    }
    ...
    public function purge($url, $conf = []) {
        return $this->request($url, "PURGE", $conf);
    }
    ...
    public function post($url, $conf = [], $asJson = false) {
        return $this->request($url, "POST", $conf, false, $asJson);
    }
    ...
    public function put($url, $conf = [], $payload = false, $asJson = false) {
        return $this->request($url, "PUT", $conf, $payload, $asJson);
    }
    ...
    public function patch($url, $conf = [], $payload = false, $asJson = false) {
        return $this->request($url, "PATCH", $conf, $payload, $asJson);
    }
    ...
    //     return $this;
    // }
    public function delete($url, $conf = [], $payload = false) {
        return $this->request($url, "DELETE", $conf, $payload);
    }
     ...
     * @date 2026-04-24
     */
    public function request($url, $method, $conf, $payload = false, $asJson = false) {
        ...
        $allowMethod = ["post", "get", "put", "delete", "patch", "purge"];
        if (!in_array($method, $allowMethod)) {
            throw new \Exception("method " . $method . " Illegal");
        }
        $headers = isset($conf['headers']) ? $conf['headers'] : [];
        if (!empty($headers)) {
        ...
        if (isset($conf['vertify']) && !$conf['vertify']) {
            $vertify = $conf['vertify'];
            $this->setOpt(CURLOPT_SSL_VERIFYPEER, $vertify);
            $this->setOpt(CURLOPT_SSL_VERIFYHOST, $vertify);
        }
